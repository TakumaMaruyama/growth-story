...
const ACTIVITY_OPTIONS = [
    { value: 'PRACTICE', label: '練習した', Icon: Waves },
    { value: 'COMPETITION', label: '大会', Icon: Medal },
    { value: 'REST', label: 'お休み', Icon: Bed },
] as const;
...
function DailyLogPageContent() {
        ...
        const fetchLog = async () => {
            ...
            if (previousRequestKey && previousRequestKey !== requestKey && dirtyRef.current) {
                ...
                if (!shouldDiscard) {
                    ...
                    setLocation(requestKeyToHref(previousRequestKey));
                    return;
                }
            ...
            try {
                ...
                const response = await fetch(`/api/daily${query}`, { credentials: 'include',
                    cache: 'no-store',
    ...
    }, [reloadToken, requestKey, requestedDate]);
    ...
    const handleDateChange = (nextDate: string) => {
        ...
        dirtyRef.current = false;
        setDirty(false);
        setLocation(`/daily?date=${encodeURIComponent(nextDate)}`);
    };
                ...
                <div className="quick-log-header">
                    <div className="quick-date-picker">
                        <label className="quick-date-control" htmlFor="date">
                            <Calendar aria-hidden="true" size={26}  />
