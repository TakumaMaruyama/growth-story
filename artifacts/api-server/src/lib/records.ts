...
export function getJSTDateTimeRange(startKey: string, endKey: string): {
    start: Date;
    end: Date;
} {
    if (!isRecordDateKey(startKey) || !isRecordDateKey(endKey) || startKey >= endKey) {
        throw new Error('Invalid JST date range');
    }
    return {
        start: fromZonedTime(`${startKey}T00:00:00`, JST),
        end: fromZonedTime(`${endKey}T00:00:00`, JST),
    };
}
