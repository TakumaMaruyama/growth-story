...
2. 保護者が共通登録URLを開く
3. 保護者が選手名、保護者氏名、選手との関係、ログインID、パスワードを入力する
4. 表示された取り扱い内容に同意して登録する

共通登録URLは複数の保護者が繰り返し利用できます。チーム外へ転送せず、URLが意図せず共有された場合は `REGISTRATION_ACCESS_TOKEN` を更新して再公開し、以前のURLを一括で無効にします。同じ選手名の再登録は表記ゆれを正規化したうえで拒否するため、登録済み・退会済みの選手は新規アカウントを作らず、管理者へ利用再開を依頼します。
